...
export interface Stage {
  ...
  title: string;
  color: string;
  pos?: number; // порядковый номер колонки (для сохранения порядка)
}
...
export interface Deal {
  ...
  objectId?: string;
  log?: DealLogEntry[];
  archived?: "done" | "lost" | null; // «Сделка завершена» / «Сделка прервана»
  createdAt: string;
}
...
export interface Product {
  ...
  purchasePrice?: number;   // цена закупки НА СКЛАДЕ (уже с долей доставки)
  basePrice?: number;       // ЧИСТАЯ цена закупки из поставки (БЕЗ доставки) — именно она подставляется при новом приходе
  stock?: number;           // кол-во на складе (для товаров)
  lastSupplyPrice?: number; // закупочная цена последней поставки (с доставкой)
}
...
export interface Supply {
  id: string;
  items: SupplyItem[];
  delivery?: number; // общая стоимость доставки, распределяется на себестоимость позиций
  date: string;
  note?: string;
  createdAt: string;
}
...
export interface Job {
  ...
  objectId: string;
  stage: JobStage;
  start?: string;  // начало работы
  deadline: string; // окончание работы
  assigneeId: string;
  comment: string;
}
...
export type Page =
  ...
  | "tasks" | "calendar" | "products" | "finance" | "settings" | "archive";
