...
/** Границы периода в миллисекундах: [начало, конец] */
export function periodBounds(p: Period): [number, number] {
  if (p.kind === "all") return [-Infinity, Infinity];
  if (p.kind === "month") {
    return [new Date(p.year, p.month, 1).getTime(), new Date(p.year, p.month + 1, 0, 23, 59, 59, 999).getTime()];
  }
  return [new Date(p.from + "T00:00:00").getTime(), new Date(p.to + "T23:59:59.999").getTime()];
}
...
export function periodName(p: Period): string {
  if (p.kind === "all") return "всё время";
  if (p.kind === "month") return `${MONTHS_FULL[p.month].toLowerCase()} ${p.year}`;
  return `${fmtD(p.from)} — ${fmtD(p.to)}`;
}

export function PeriodFilter({ value, onChange }: { value: Period; onChange: (p: Period) => void }) {
  const [open, setOpen] = useState(false);
  const [mode, setMode] = useState<"all" | "month" | "range">(value.kind);
  const now = new Date();
  const [vy, setVy] = useState(value.kind === "month" ? value.year : now.getFullYear());
  const [rf, setRf] = useState(value.kind === "range" ? value.from.slice(0, 10) : "");
  const [rt, setRt] = useState(value.kind === "range" ? value.to.slice(0, 10) : "");
  const ref = useClickOutside(() => setOpen(false));

  const label = value.kind === "all"
    ? "За всё время"
    : value.kind === "month"
  ...
  return (
    <div className="relative" ref={ref}>
      <button className="btn btn-ghost btn-sm" onClick={() => setOpen(!open)} title="Фильтр по периоду">
        <Icon name="calendar" size={14} />
        {label}
        <Icon name="chevD" size={13} className={`transition-transform ${open ? "rotate-180" : ""}`} />
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-2 z-40 card p-3 w-[280px] anim-pop" style={{ boxShadow: "var(--shadow-lg)" }}>
          <div className="seg w-full !p-1 mb-3">
            <button className={mode === "all" ? "on" : ""} onClick={() => { setMode("all"); onChange({ kind: "all" }); setOpen(false); }}>Всё</button>
            <button className={mode === "month" ? "on" : ""} onClick={() => setMode("month")}>Месяц</button>
            <button className={mode === "range" ? "on" : ""} onClick={() => setMode("range")}>Период</button>
          </div>
          {mode === "month" && (
            <div>
              <div className="flex items-center justify-between mb-2">
                <button className="icon-btn !w-7 !h-7" onClick={() => setVy(vy - 1)} aria-label="Предыдущий год"><Icon name="chevL" size={14} /></button>
                <span className="text-[13px] font-extrabold font-display">{vy}</span>
                <button className="icon-btn !w-7 !h-7" onClick={() => setVy(vy + 1)} aria-label="Следующий год"><Icon name="chevR" size={14} /></button>
              </div>
              <div className="grid grid-cols-3 gap-1.5">
                {MONTHS_SHORT.map((m, i) => {
                  const sel = value.kind === "month" && value.year === vy && value.month === i;
                  return (
                    <button key={m} onClick={() => { onChange({ kind: "month", year: vy, month: i }); setOpen(false); }}
                      className="py-1.5 rounded-lg text-[12px] font-bold transition-colors cursor-pointer"
                      style={sel ? { background: "var(--brand)", color: "#fff" } : { color: "var(--muted)", background: "transparent" }}
          ...
          {mode === "range" && (
            <div className="space-y-2">
              <div>
                ...
                <input type="date" className="input !py-1.5 !text-[12.5px]" value={rf} onChange={(e) => setRf(e.target.value)} />
              </div>
              <div>
                <label className="label">По дату</label>
                <input type="date" className="input !py-1.5 !text-[12.5px]" value={rt} onChange={(e) => setRt(e.target.value)} />
              </div>
              <button className="btn btn-primary btn-sm w-full" disabled={!rf || !rt}
                onClick={() => { if (rf && rt) { onChange({ kind: "range", from: rf, to: rt }); setOpen(false); } }}>
                Применить
              </button>
            </div>
          )}
