...
export function Sidebar() {
  ...
  const activeDeals = deals.filter((d) => d.stageId !== "st6" && !d.archived).length;
...
function Bell() {
  ...
  const iconFor = (type: string) =>
    type === "overdue" ? "alert" : type === "task" ? "checkSq" : type === "payment" ? "ruble" : type === "booking" ? "box" : "bell";
  const colorFor = (type: string) =>
    type === "overdue" ? "var(--red)" : type === "task" ? "var(--brand)" : type === "payment" ? "var(--green)" : type === "booking" ? "#c9a227" : "var(--muted)";
            ...
            {mine.length === 0 && (
              <div className="px-3 py-10 text-center">
                ...
                <div className="text-[11.5px] mt-1" style={{ color: "var(--muted)" }}>
                  Здесь появятся назначенные вам задачи, брони товаров и сигналы о просрочках
                </div>
              </div>
            )}
            {mine.map((n) => (
