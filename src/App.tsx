...
import type { ModalState, Role, User } from "./types";
import {
  fetchAll, subscribeRemote, setSyncErrorHandler, uploadAll, pushUpsert,
...
import Warehouse, { ProductModal, SupplyModal, SupplyHistory, BookingModal } from "./pages/Warehouse";
...
function renderModal(m: ModalState) {
  ...
  switch (m.type) {
    ...
    case "supply": return <SupplyModal key={m.id ?? "new"} supplyId={m.id} />;
    case "supply-history": return <SupplyHistory />;
    ...
    case "deal": return <DealModal key={m.id ?? "new"} id={m.id} stageId={m.stageId} />;
...
async function enterCloud(user: any, nameHint?: string, roleHint?: string) {
  ...
  try {
    ...
    const prof: User = {
      ...
      phone: profile.phone ?? undefined, email: profile.email ?? undefined,
      blocked: !!profile.blocked, overrides: profile.overrides ?? undefined,
    };
    if (prof.blocked) {
      useCRM.setState({ loggedIn: false, mode: "demo", cloudState: "idle", cloudUser: null, modal: [] } as any);
      crm.toast("Аккаунт заблокирован директором — вход запрещён", "alert");
      await authLogout();
      return;
    }
    /* сотрудник должен быть в списке, даже если база пока пустая */
    if (!useCRM.getState().users.some((u) => u.id === prof.id)) {
      useCRM.setState((s) => ({ users: [...s.users, prof] }));
    }
    ...
    // если в базе нет колонок воронки — выгружаем текущий полный набор с порядком,
    // чтобы первая же правка не «потеряла» остальные столбцы
    if (!(data.stages?.length)) {
      useCRM.getState().stages.forEach((st, i) => pushUpsert("stages", { ...st, pos: i }));
    }
    // настройки ИИ (маска — ключ остаётся в базе)
    fetchAiConfig().then((cfg) => cfg && useCRM.getState().setAiConfig(cfg)).catch(() => {});
...
export default function App() {
  ...
  /* обработчик ошибок синхронизации → баннер (заметно и не спамит) */
  useEffect(() => {
    setSyncErrorHandler((msg) => useCRM.getState().setSyncError(msg));
  }, []);

  /* автоподключение после перезапуска страницы/браузера.
     Источник правды — САМА сессия Supabase, а не запись в браузере:
     пока сессия жива, пользователь гарантированно возвращается в облако. */
  ...
  /* фоновая проверка просроченных задач + досинхронизация данных */
  useEffect(() => {
    if (!loggedIn) return;
    ...
    const pull = setInterval(() => {
      if (useCRM.getState().mode !== "cloud") return;
      fetchAll()
        .then(({ rows }) => { useCRM.getState().hydrate(rows); useCRM.getState().setSyncError(null); })
        .catch(() => { /* молча — баннер покажет ошибку при записи */ });
    }, 20000);
    return () => { clearInterval(timer); clearInterval(pull); };
  }, [loggedIn]);

  if (booting) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4" style={{ background: "var(--sidebar)" }}>
        <span className="w-14 h-14 rounded-2xl flex items-center justify-center" style={{ background: "rgba(232,163,61,0.14)" }}>
          <Icon name="crane" size={30} className="text-[#e8a33d]" sw={1.9} />
        </span>
        <div className="flex items-center gap-2.5 text-[13.5px] font-bold" style={{ color: "#c3cfc6" }}>
          <span className="w-4 h-4 rounded-full border-2 inline-block" style={{ borderColor: "#e8a33d", borderRightColor: "transparent", animation: "spin 0.7s linear infinite" }} />
  ...
  const Page = { home: Home, sales: Sales, clients: Clients, objects: Objects, jobs: Jobs, tasks: Tasks, calendar: Calendar, products: Warehouse, finance: Finance, settings: Settings, archive: Archive }[page];
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* ИИ-ассистент (перетаскиваемая кнопка + панель чата) */}
      <AiAssistant />
...
function LoginScreen({ onInviteDone }: { onInviteDone?: () => void }) {
  ...
  useEffect(() => {
    ...
    lookupInvite(token)
      ...
      .catch(() => { /* нет связи — оставляем обычный вход */ });
          ...
          {tab === "cloud" ? (
            <div className="anim-fade">
              ...
              <p className="text-[13px] mt-1.5 mb-5" style={{ color: "var(--muted)" }}>
                {invite
                  ? `Вас пригласили на должность «${invite.role}» — осталось задать имя и пароль`
                  : "Общие данные бригады — сделки, объекты и финансы — в облаке Supabase. Регистрация — только по приглашению директора."}
              </p>
                ...
                {invite && (
                  <div className="flex items-start gap-2.5 p-3 rounded-[10px] anim-pop" style={{ background: "var(--brand-soft)", border: "1px solid color-mix(in srgb, var(--brand) 35%, var(--line))" }}>
                    ...
                    <div className="text-[12px] font-semibold leading-relaxed" style={{ color: "var(--brand)" }}>
                      Должность <b>«{invite.role}»</b> и email закреплены за этим приглашением.
                    </div>
                  </div>
                )}
                {(invite || true) && (
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          {roles.map((r) => (
                            <button key={r.id} onClick={() => setRole(r.name)}
                              className="py-2 rounded-[9px] border-2 text-[12px] font-bold cursor-pointer transition-all truncate px-1"
                              style={{ borderColor: role === r.name ? r.color : "var(--line)", background: role === r.name ? `${r.color}22` : "var(--panel)", color: role === r.name ? r.color : "var(--muted)" }}>
                              {r.name}
                            </button>
                          ))}
                        </div>
...
function SeedAsk() {
  ...
  const clean = () => {
    useCRM.getState().wipeLocal();
    crm.toast("Чистый лист: справочники сохранены, данные пусты", "bell");
    useCRM.setState({ askSeed: false } as any);
  };
