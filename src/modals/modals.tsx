...
export function ObjectModal({ id, dealId }: { id?: string; dealId?: string }) {
  ...
  const can = useCan();
  const canEditObj = can("objects.edit");
  const canCreateObj = can("objects.create");
  const canWorks = can("objects.works");
  const canPay = can("finance.create");
...
export function JobModal({ id, objectId }: { id?: string; objectId?: string }) {
  ...
  const defSt = () => toLocal(new Date().toISOString());
  const [f, setF] = useState<Job>(existing ?? { id: uid(), title: "", objectId: objectId ?? objects[0]?.id ?? "", stage: "plan", start: defSt(), deadline: fromLocal(defDl()), assigneeId: currentUserId, comment: "" });
  const [stLocal, setStLocal] = useState(existing?.start ? toLocal(existing.start) : defSt());
  ...
  const save = () => {
    ...
    const startIso = fromLocal(stLocal);
    const deadlineIso = fromLocal(dlLocal);
    if (+new Date(startIso) > +new Date(deadlineIso)) { toast("Начало работы не может быть позже окончания", "alert"); return; }
    saveJob({ ...f, title: f.title.trim(), start: startIso, deadline: deadlineIso }, !existing);
      ...
      <fieldset disabled={readOnly} className="border-0 p-0 m-0 min-w-0 space-y-3.5">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <input type="datetime-local" className="input" value={stLocal} onChange={(e) => setStLocal(e.target.value)} />
          </div>
          <div>
...
export function PaymentModal({ kind: initKind = "income", dealId, clientId: initClient, payId }: { kind?: "income" | "expense"; dealId?: string; clientId?: string; payId?: string }) {
  const { payments, deals, clients, expenseCats, closeModal, addPayment, updatePayment, deletePayment, toast } = useCRM();
  ...
  const [category, setCategory] = useState(editing?.category ?? expenseCats[0] ?? "Прочее");
  const [note, setNote] = useState(editing?.note ?? "");
  const cats = editing?.category && !expenseCats.includes(editing.category)
    ? [editing.category, ...expenseCats]
    : expenseCats;
