...
export default function Settings() {
  const crm = useCRM();
  const can = useCan();
  const { users, currentUserId, theme, setTheme, logout, leadSources, addSource, removeSource, objectStatuses, addObjectStatus, removeObjectStatus, notifPrefs, setNotifPref, roles, toast, loginDemo, mode, openModal } = crm;
            ...
            <button key={r.id} onClick={() => select(r)}
              ...
              <span className="flex-1 min-w-0">
                ...
                <span className="block text-[10.5px]" style={{ color: "var(--muted)" }}>
                  {memberCount(r.name)} чел. · {r.permissions.length} прав
                </span>
              </span>
