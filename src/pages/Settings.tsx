...
export default function Settings() {
  const crm = useCRM();
  const can = useCan();
  const { users, currentUserId, theme, setTheme, logout, leadSources, addSource, removeSource, expenseCats, addExpenseCat, removeExpenseCat, objectStatuses, addObjectStatus, removeObjectStatus, notifPrefs, setNotifPref, roles, toast, loginDemo, mode, openModal } = crm;
  ...
  return (
    <div className="p-6 max-w-[980px] mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="card p-5">
          ...
          <div className="flex items-center gap-3.5 p-3.5 rounded-xl mb-4" style={{ background: "var(--panel2)" }}>
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[14.5px] font-extrabold">{me.name}</div>
        ...
        <div className="flex flex-col gap-4">
          {/* notifications — доступно по праву «Настройка уведомлений» (по умолчанию есть у всех, директор может отозвать) */}
...
function DangerZone() {
  ...
  const doWipe = async () => {
    ...
    try {
      ...
      /* аккаунт удалён из базы вместе со всеми — завершаем и сессию Supabase,
         чтобы следующий, кто откроет CRM, зарегистрировался заново как Директор */
      setTimeout(async () => {
...
function EmployeesCard() {
  ...
  /* чью карточку разрешено открыть */
...
function AiCard() {
  ...
  /* ключ ИИ живёт только в облаке — в демо-режиме настроить его нельзя */
  ...
  /* любое написание приводится к техническому виду API («3.5 Flash Lite» → «gemini-3.5-flash-lite») */
...
function RolesConstructor() {
  ...
  /* область действия: denied → granted → own (если есть) → denied */
