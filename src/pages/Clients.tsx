...
import { useCRM, dealTotal, dFmt } from "../store";

export default function Clients() {
  const { clients, deals, payments, openModal } = useCRM();
      ...
      <div className="card overflow-hidden">
        {list.length === 0 ? (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {list.map((c) => {
                const cd = deals.filter((d) => d.clientId === c.id && !d.archived);
                ...
                return (
                  <tr key={c.id} className="row-hover border-t" style={{ borderColor: "var(--line)" }} onClick={() => openModal({ type: "client", id: c.id })}>
                    ...
                    <td className="px-4 py-3 text-[13px] font-extrabold whitespace-nowrap">{sum > 0 ? sum.toLocaleString("ru-RU") + " ₽" : "—"}</td>
