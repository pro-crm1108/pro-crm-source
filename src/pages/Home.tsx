...
import { Progress, ToneChip, PeriodFilter, periodBounds, periodName, type Period } from "../components/ui";
import { useCRM, money, moneyShort, dealTotal, dealPaid, dFmt, startOfDay, dueLabel } from "../store";

export default function Home() {
  const { deals, payments, objects, tasks, users, clients, stages, openModal, setPage, saveTask, toast } = useCRM();
  ...
  const todayTasks = tasks
    .filter((t) => !t.done && +new Date(t.due) < startOfDay(1))
  ...
  const upcoming = deals
    ...
    .filter((x) => x.rest > 0)
    .sort((a, b) => stages.findIndex((s) => s.id === a.d.stageId) - stages.findIndex((s) => s.id === b.d.stageId))
    .slice(0, 4);
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 stagger">
        {tiles.map((t) => (
          <div key={t.label} className="card p-5 relative overflow-hidden group hover:-translate-y-1 transition-all">
            ...
            <div className="font-display text-[22px] font-bold mt-2.5 leading-none" style={{ color: t.label === "Прибыль" ? t.tone : "var(--ink)" }}>{t.val}</div>
      ...
      <div className="grid gap-4 mt-4 lg:grid-cols-[minmax(0,6fr)_minmax(0,4fr)] grid-cols-1">
        ...
        <div className="card p-5 anim-page">
          ...
          {deals.length > 0 && (
            <div className="flex items-center justify-between mt-5 pt-4 border-t" style={{ borderColor: "var(--line)" }}>
              <span className="text-[12px] font-semibold" style={{ color: "var(--muted)" }}>Конверсия в договор и оплату</span>
              <span className="font-display text-[14px] font-bold" style={{ color: "var(--green)" }}>
                {Math.round(((funnel[4]?.count ?? 0) + (funnel[5]?.count ?? 0)) / Math.max(1, deals.filter((d) => !d.archived).length) * 100)}%
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-4 min-w-0">
          ...
          <div className="card p-5 anim-page flex-1" style={{ animationDelay: "0.05s" }}>
            ...
            {todayTasks.length === 0 ? (
              <div className="text-[13px] font-semibold py-6 text-center" style={{ color: "var(--faint)" }}>На сегодня задач нет</div>
            ) : (
                ...
                {todayTasks.slice(0, 5).map((t) => {
                  ...
                  return (
                    <div key={t.id} className="flex items-center gap-2.5 p-2 rounded-[9px] row-hover group">
                      ...
                      {u && (
                        <span className="w-6 h-6 rounded-full flex items-center justify-center text-[9px] font-extrabold text-white flex-none" style={{ background: u.color }}>
          ...
          <div className="card p-5 anim-page" style={{ animationDelay: "0.1s" }}>
            ...
            {upcoming.length === 0 ? (
              <div className="text-[13px] font-semibold py-6 text-center" style={{ color: "var(--faint)" }}>Все сделки оплачены</div>
            ) : (
              <div className="space-y-2.5">
                {upcoming.map(({ d, rest }) => {
