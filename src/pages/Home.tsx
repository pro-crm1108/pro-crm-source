...
import { Avatar, Progress, ToneChip, Empty } from "../components/ui";
...
export default function Home() {
  ...
  const fin = useMemo(() => {
    const totalDeals = deals.reduce((a, d) => a + dealTotal(d), 0);
    const income = payments.filter((p) => p.kind === "income").reduce((a, p) => a + p.amount, 0);
    const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0, 0, 0, 0);
    const incomeMonth = payments.filter((p) => p.kind === "income" && +new Date(p.date) >= +monthStart).reduce((a, p) => a + p.amount, 0);
    const expense = payments.filter((p) => p.kind === "expense").reduce((a, p) => a + p.amount, 0);
    const expenseMonth = payments.filter((p) => p.kind === "expense" && +new Date(p.date) >= +monthStart).reduce((a, p) => a + p.amount, 0);
    const remainder = deals.reduce((a, d) => a + Math.max(0, dealTotal(d) - dealPaid(d, payments)), 0);
    const paidOff = deals.filter((d) => dealTotal(d) > 0 && dealPaid(d, payments) >= dealTotal(d)).length;
    const fullyPaidSum = deals.filter((d) => dealTotal(d) > 0 && dealPaid(d, payments) >= dealTotal(d)).reduce((a, d) => a + dealTotal(d), 0);
    return { totalDeals, income, incomeMonth, expense, expenseMonth, remainder, paidOff, fullyPaidSum, profit: incomeMonth - expenseMonth };
  }, [deals, payments]);
  ...
  const tiles = [
    { label: "Сумма сделок", val: money(fin.totalDeals), sub: `${deals.length} сделок в работе`, icon: "kanban", tone: "var(--brand)" },
    { label: "Получено", val: money(fin.incomeMonth), sub: "в этом месяце", icon: "ruble", tone: "var(--green)" },
    { label: "Остаток", val: money(fin.remainder), sub: "долг клиентов по сделкам", icon: "clock", tone: "var(--amber)" },
    { label: "Расходы", val: money(fin.expenseMonth), sub: "в этом месяце", icon: "wallet", tone: "var(--red)" },
    { label: "Прибыль", val: money(fin.profit), sub: "доходы − расходы за месяц", icon: "funnel", tone: fin.profit >= 0 ? "var(--green)" : "var(--red)" },
