...
import { Seg, Empty } from "../components/ui";
...
export default function Finance() {
  ...
  const filtered = useMemo(() => {
    const nowT = Date.now();
    const d30 = nowT - 30 * 86400000;
    const monthStart = new Date(); monthStart.setDate(1); monthStart.setHours(0, 0, 0, 0);
    return payments.filter((p) => {
      const t = +new Date(p.date);
      if (period === "30") return t >= d30;
  ...
  const debt = deals.reduce((a, d) => a + Math.max(0, dealTotal(d) - dealPaid(d, payments)), 0);

  const tiles = [
    { label: "Доход", val: moneyShort(income), icon: "ruble", tone: "var(--green)", note: `${filtered.filter((p) => p.kind === "income").length} поступлений` },
    { label: "Расход", val: moneyShort(expense), icon: "wallet", tone: "var(--red)", note: `${filtered.filter((p) => p.kind === "expense").length} операций` },
    { label: "Прибыль", val: moneyShort(income - expense), icon: "funnel", tone: income - expense >= 0 ? "var(--green)" : "var(--red)", note: income > 0 ? `маржа ${Math.round(((income - expense) / income) * 100)}%` : "—" },
    ...
    <div className="p-6 max-w-[1250px] mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2.5">
          <Seg value={period} onChange={setPeriod} options={[{ v: "all", t: "Всё время" }, { v: "30", t: "30 дней" }, { v: "month", t: "Месяц" }]} />
