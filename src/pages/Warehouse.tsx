...
export default function Warehouse() {
  ...
  const goods = useMemo(() => {
    ...
    /* supplies идут от новых к старым — первая найденная поставка и есть последняя */
...
export function SupplyModal({ supplyId }: { supplyId?: string }) {
  ...
  /* только заполненные строки участвуют в расчётах */
  ...
  /* надбавка за доставку на каждую единицу товара */
      ...
      <div className="p-5">
        ...
        <div className="space-y-2">
          {rows.map((r, k) => {
            ...
            const effUnit = r.price + perUnit; /* цена + доля доставки */
...
export function SupplyHistory() {
  ...
  /* ПИН-защита для обоих действий: редактирования и удаления */
  const [pinFor, setPinFor] = useState<{ action: "edit" | "delete"; supply: Supply } | null>(null);
  const [pin, setPin] = useState("");
  const [pinErr, setPinErr] = useState("");
  const [busy, setBusy] = useState(false);

  /* новый приход всегда сверху: сортируем по точному времени создания записи
     (для старых записей без createdAt — по дате поставки) */
  ...
  const confirmPin = async () => {
    ...
    try {
      ...
      if (action === "edit") {
        /* окно редактирования открывается ПОВЕРХ истории — история остаётся под ним */
...
export function BookingModal({ dealId, productId }: { dealId: string; productId: string }) {
  ...
  return (
    <Modal onClose={closeModal} width={560} icon="layers" title="Бронь товара"
      ...
      <div className="p-5">
        {/* шапка: название слева, приглушённая кнопка «Отгрузили» справа */}
