...
export default function Sales() {
  const crm = useCRM();
  const { deals, stages, clients, users, openModal, renameStage, moveDeal, reorderStages, payments, currentUserId } = crm;
  const scope = usePermScope();
  const can = useCan();
  const [mode, setMode] = useState<"kanban" | "list">("kanban");
  const [dragId, setDragId] = useState<string | null>(null);
  const [overCol, setOverCol] = useState<string | null>(null);
  /* перетаскивание самих колонок воронки */
  const [dragStage, setDragStage] = useState<string | null>(null);
  const [overStage, setOverStage] = useState<string | null>(null);

  /* «Только свои» → видны сделки, где пользователь — ответственный.
     Архивные сделки (завершённые/прерванные) в воронке не показываем. */
  const onlyMine = scope("deals.viewCard") === "own";
  const visibleDeals = useMemo(
    () => (onlyMine ? deals.filter((d) => d.ownerId === currentUserId) : deals).filter((d) => !d.archived),
        ...
        <div className="flex-1 overflow-x-auto overflow-y-hidden pb-2 -mx-1 px-1">
          <div className="flex gap-3 h-full min-w-max">
            {stages.map((st) => {
              ...
              return (
                <div key={st.id}
                  className={`w-[280px] flex-none flex flex-col rounded-xl border transition-colors ${overCol === st.id ? "kanban-col-drop" : ""} ${overStage === st.id ? "kanban-col-drop" : ""}`}
                  style={{ background: "var(--panel2)", borderColor: "var(--line)" }}
                  onDragOver={(e) => { e.preventDefault(); if (dragStage) setOverStage(st.id); else setOverCol(st.id); }}
                  onDragLeave={() => { setOverCol((c) => (c === st.id ? null : c)); setOverStage((c) => (c === st.id ? null : c)); }}
                  onDrop={() => {
                    if (dragStage) {
                      /* переставляем саму колонку воронки */
                      if (dragStage !== st.id && canStructure) {
                        reorderStages(dragStage, st.id);
                        crm.toast("Порядок колонок обновлён", "bell");
                      }
                      setDragStage(null); setOverStage(null);
                      return;
                    }
                    if (dragId) { const dl = deals.find((d) => d.id === dragId); if (dl && canEditDeal(dl)) { moveDeal(dragId, st.id); crm.toast(`«${dl.title}» → «${st.title}»`, "bell"); } else crm.toast("Нет права менять стадию этой сделки", "alert"); }
                    setDragId(null); setOverCol(null);
                  }}>
                  <StageHeader st={st} count={list.length} sum={sum} canStructure={canStructure}
                    onRename={(t) => renameStage(st.id, t)}
                    onDragStart={canStructure ? () => setDragStage(st.id) : undefined}
                    onDragEnd={() => { setDragStage(null); setOverStage(null); }}
                    dragging={dragStage === st.id} />
...
function StageHeader({ st, count, sum, canStructure, onRename, onDragStart, onDragEnd, dragging }: {
  st: any; count: number; sum: number; canStructure: boolean; onRename: (t: string) => void;
  ...
  return (
    <div
      className={`px-3.5 pt-3 pb-2.5 flex items-center gap-2 flex-none group/head ${dragging ? "opacity-50" : ""}`}
      draggable={!!onDragStart && !edit}
      onDragStart={(e) => { if (!onDragStart || edit) { e.preventDefault(); return; } e.dataTransfer.effectAllowed = "move"; onDragStart(); }}
      onDragEnd={onDragEnd}
      title={onDragStart ? "Перетащите, чтобы изменить порядок колонок" : undefined}
    >
      {canStructure && (
        <span className="flex-none cursor-grab active:cursor-grabbing opacity-30 group-hover/head:opacity-70 transition-opacity" style={{ color: "var(--muted)" }} title="Порядок колонок">
