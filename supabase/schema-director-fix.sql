...
-- 1) whoami() — вызывается ИЗ CRM и честно показывает, что база «думает» о вас:
...
-- 2) save_ai_config — при отказе сама говорит причину.
...
begin
  ...
  if v_name is null then
    raise exception 'Не получается сохранить: запись вашего сотрудника не найдена в базе (id: %).', coalesce(auth.uid()::text, 'нет входа');
  end if;
  ...
  if not v_is_director then
    raise exception 'Менять настройки ИИ может только директор. Ваша должность в базе: «%» (имя: %).', coalesce(v_role, '—'), v_name;
  end if;
