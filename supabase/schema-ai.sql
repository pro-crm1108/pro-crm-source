...
--    Пустой ключ = «оставить старый» (чтобы не вводить ключ
--    заново при смене провайдера или модели).
-- ------------------------------------------------------------
...
begin
  -- Надёжно определяем директора: подходит ЛЮБОЕ из трёх условий
  -- (по имени «Директор», по системной записи role-director,
  --  или по праву «Настройка ИИ» / «Полный доступ к CRM»)
  ...
  -- приводим модель к техническому виду: «3.5 Flash Lite» → «gemini-3.5-flash-lite»
...
-- ------------------------------------------------------------
-- 3) Сам разговор с ИИ: принимает сообщения
--    [{role: system|user|assistant, content: ...}],
--    обращается к провайдеру и возвращает текст ответа.
-- ------------------------------------------------------------
...
begin
  ...
  -- страховка: чистим модель и ключ от пробелов/регистра,
  -- чтобы в адрес запроса не попало «3.5 Flash Lite» и URL не сломался
  ...
  if v_provider = 'gemini' then
    ...
    -- превращаем сообщения в формат Gemini: system → systemInstruction,
    -- user/assistant → contents (подряд идущие роли склеиваем)
    for rec in
    ...
    -- ключ уже в адресе запроса; отправка без заголовков —
    -- совместимо с любой версией HTTP-модуля Supabase
